...
public class BarraDeProgresso : MonoBehaviour {
    public StatusBarraEspecial statusBarra;
	...
	void Start () {
        statusBarra = this.gameObject.GetComponent<StatusBarraEspecial>();
	}
	...
	void Update () {
        ...
        if (porcentagemAtual < maxPorcentagem)
        {
            porcentagemAtual += Time.deltaTime * 1.67f; // se a porcentagem for menor dq o valor max(100%) continua aumentando a barra, o 1.67 é o valor de 100/60 (para fazer q com 100% tenha se passado 60s que é o tempo do especial)
        }
        else {
        ...
        if (contador >= 60.0f && Input.GetButtonDown("Jump")) // se chegar a 60s e apertar o SPACE a porcentagem da barra é resetada e sua contagem tb
        {
            porcentagemAtual = 0f;
            contador = 0f;
        }
